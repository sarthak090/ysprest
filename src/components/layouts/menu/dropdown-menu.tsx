        ...
        className={cn(
          'flex h-11 shrink-0 items-center text-sm font-semibold text-heading focus:outline-none md:text-base xl:px-4',
          {
            'rounded-lg border border-border-200 bg-gray-50 px-3':
              variant === 'minimal',
            'rounded border-border-200 bg-light xl:min-w-150  xl:text-accent':
              variant === 'colored',
          },
          className
        )}
      >
        {({ open }) => (
          <>
            ...
            <span className="ml-4 flex  gap-3 pt-1">
              {variant === 'colored' && (
                <CaretDown
...
const DropdownMenu: React.FC<GroupsDropdownMenuProps> = ({
  variant,
  groups,
  defaultGroup,
}) => {
  return (
    <GroupsMenu groups={groups} defaultGroup={defaultGroup} variant={variant} />
  );
};
