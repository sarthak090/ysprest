...
> = async ({ locale }) => {
  ...
  let pageType: string | undefined = 'spritiual'; //change page Layout from this
  ...
  const bestSelllerUrl =
    process.env.NEXT_PUBLIC_REST_API_ENDPOINT + `best-sellers`;

  const bestSelllerData = await fetch(bestSelllerUrl).then((r) => r.json());
  ...
  return {
    props: {
      variables: {
        ...
        bestSelllerProducts: bestSelllerData,
        posts: posts,
        types: {
          type: pageType,
        },
